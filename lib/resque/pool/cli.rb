...
where [options] are:
      ...
      def daemonize
        fail 'First fork failed' if (pid = fork) == -1
        exit unless pid.nil?
        Process.setsid
        fail 'Second fork failed' if (pid = fork) == -1
        exit unless pid.nil?
      end
      ...
      def manage_pidfile(pidfile)
        ...
        if File.exist? pidfile
          if process_still_running? pidfile
            fail "Pidfile already exists at #{pidfile} and process is still running."
          else
            File.delete pidfile
          end
        else
        ...
        at_exit do
          File.delete pidfile if Process.pid == pid
        end
      end
      ...
      def set_pool_options(opts)
        Resque::Pool.handle_winch = true if opts[:daemon]
